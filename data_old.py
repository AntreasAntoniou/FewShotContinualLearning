...
class FewShotLearningDatasetParallel(Dataset):
    def __init__(self, args, model=None):
        ...
        self.num_target_samples = args.num_samples_per_target_class
        ...
        self.num_samples_per_support_class = args.num_samples_per_support_class
    ...
    def get_set(self, dataset_name, seed, augment_images=False):
        ...
        for class_idx, class_entry in enumerate(selected_classes):
            choose_samples_list = rng.choice(self.dataset_size_dict[dataset_name][class_entry],
                                             size=self.num_samples_per_support_class + self.num_target_samples, replace=False)
            x_current_class = []
            y_current_class = []
            ...
            support_set_images.extend(x_current_class[:self.num_samples_per_support_class])
            support_set_labels.extend(y_current_class[:self.num_samples_per_support_class])
            support_set_original_labels.extend(y_original_class[:self.num_samples_per_support_class])

            target_set_images.extend(x_current_class[self.num_samples_per_support_class:])
            target_set_labels.extend(y_current_class[self.num_samples_per_support_class:])
            target_set_original_labels.extend(y_original_class[self.num_samples_per_support_class:])
        ...
        return support_set_images.unsqueeze(0), target_set_images.unsqueeze(0), support_set_labels.unsqueeze(0).long(), target_set_labels.unsqueeze(0).long()
