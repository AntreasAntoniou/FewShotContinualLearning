...
def get_experiment_config_from_script_file(script_name):
    ...
    config_file_name = execution_line.split("experiment_config_files/")[1].split(" ")[0]
    config_file_name = "{}/{}".format("../experiment_config_files", config_file_name).replace("\n", "")
...
def get_stats_from_experiment_name(experiment_name):
    total_epochs_done, test_acc, test_loss, val_acc, val_loss, train_acc, train_loss = 0, 0, 0, 0, 0, 0, 0
            ...
            if file.endswith("summary.csv"):
                ...
                try:
                    if stats_dict is not None:
                        train_acc = np.max(stats_dict['train_acc_mean'])
                        train_loss = np.min(stats_dict['train_loss_mean'])
                        val_acc = np.max(stats_dict['val_acc_mean'])
