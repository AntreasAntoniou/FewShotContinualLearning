...
for (k_shot, batch_size) in [(1, 1), (5, 1)]:
    for use_channel_wise_attention in [True, False]:
        for classifier_type in ['vgg-based', 'densenet-embedding-based']:
                    ...
                    configs_list.append(
                        config(dataset_name="mini_imagenet", num_classes=5, samples_per_class=k_shot,
                               ...
                               exclude_param_string=string_generator(
                                   ["None"]),
                               experiment_name='standard_5_way_{}_{}_shot_V2_{}_{}_LSLR_conditioned'.format(
                                   k_shot, classifier_type, "_".join([],), output_dim), learnable_bn_beta=True,
                               ...
                               output_spatial_dimensionality=output_dim, use_channel_wise_attention=use_channel_wise_attention, classifier_type=classifier_type))
                    for conditional_information in experiment_conditional_information_config:
                        configs_list.append(
                            config(dataset_name="mini_imagenet", num_classes=5, samples_per_class=k_shot,
                                   ...
                                   exclude_param_string=string_generator(
                                       ["None"]),
                                   experiment_name='intrinsic_5_way_{}_{}_shot_V2_{}_{}_LSLR_conditioned'.format(
                                       k_shot, classifier_type, "_".join(conditional_information), output_dim), learnable_bn_beta=True,
                                   learnable_bn_gamma=True,
                                   num_stages=4, num_blocks_per_stage=0,
                                   inner_loop_optimizer_type='LSLR', learnable_betas=False, learnable_learning_rates=True,
                                   output_spatial_dimensionality=output_dim, use_channel_wise_attention=use_channel_wise_attention, classifier_type=classifier_type))
