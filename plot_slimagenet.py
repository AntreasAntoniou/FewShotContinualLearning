...
file_path = 'slimagenet_results.csv'
...
if args.use_legend:
    plt.legend(handlelength=1, fontsize=11)

plt.ylim(bottom=0.0, top=50.0)
#plt.xlim(ymax = 250, ymin = 25)
plt.title("SlimageNet (CCI=" + str(args.cci) + "; Overwrite=" + str(with_overwrite) + ")") # for title
...
plt.tight_layout()
plt.savefig('./figure_slimagenet_' + str(args.cci) + '_' + str(with_overwrite) + '.png')
